...
/* ------------------------------- AUDIO --------------------------------- */
/* Central audio controller. Configured for very slow, crystal-clear narration. */
export const AudioService = {
  ...
  /* say(): speak phrases with a very slow, clear, child-friendly voice */
  say: function (text, opts) {
    ...
    try {
      ...
      /* Very slow, deliberate speech rate (0.60 default for maximum clarity for ages 3-6) */
      u.rate = opts.rate ? Math.min(opts.rate, 0.68) : 0.60;
      u.pitch = opts.pitch || 1.10;
      u.volume = 1;
  ...
  _voice: function () {
    try {
      ...
      for (var k = 0; k < this._voices.length; k++) {
        if (/^en/.test(this._voices[k].lang)) return this._voices[k];
      }
    } catch (e) {}
    return null;
  },
